        ...
        if correct_reg_via_trackpy: #assuming there are only x and y dim; correcting sift wrt anchors[0,:,:]
            ...
            if not correct_reg_detect_in_all:# collect coordinates setected in anchors[0,:,:]
                if (after_correction_decrease_sep_by > 0) or (after_correction_decrease_prc_by > 0):
                    locs = trackpy.locate(anchors[0,:,:], diameter=trackpy_diam_detect, percentile=trackpy_prc-after_correction_decrease_prc_by, separation=trackpy_diam_detect+1-after_correction_decrease_sep_by)
...
def load_tiles_to_extract_spots_parallel(tifs_path, channels_info, C, R,
                                        tile_names, tiles_info, tiles_to_load,
                                        spots_params, ind_cy_move_forward_by=0, anchor_available=True, fake_anchor_prc=95, 
                                        fake_anchor_gauss_sigma=None, fake_anchor_from_top_hat=False, anchors_top_hat=False,
    ...
    def tile_function(x_ind,y_ind):
        ...
        for ind_cy in range(R):
            for ind_ch in range(len(channels_info['channel_names'])):
                if channels_info['channel_names'][ind_ch] != 'DAPI':  # no need for dapi
                    try:
                        imgs[:, :, ind_ch, ind_cy] = tifffile.imread(tifs_path + tiles_info['filename_prefix'] + channels_info['channel_names'][ind_ch] + '_c0' + str(ind_cy + 1 + ind_cy_move_forward_by) + '_' + tile_name + '.tif').astype(np.float32)
                    except:
                        imgs[:, :, ind_ch, ind_cy] = tifffile.imread(tifs_path + tiles_info['filename_prefix'] + tile_name + '_c0' + str(ind_cy + 1 + ind_cy_move_forward_by) + '_' + channels_info['channel_names'][ind_ch] + '.tif').astype(np.float32)
        ...
        for ind_cy in range(R):
            for ind_ch in range(C):
                imgs_coding_tophat[:, :, ind_ch, ind_cy] = white_tophat(imgs_coding[:, :, ind_ch, ind_cy],disk(spots_params['spot_diam_tophat']))
        ...
        if anchor_available:
            ...
            if fake_anchor_from_top_hat:
                ...
                imgs_for_fake_anchor = imgs_coding    
            imgs_for_fake_anchor_norm = (imgs_for_fake_anchor - np.min(imgs_for_fake_anchor, axis=(0, 1),keepdims=True)) / (np.percentile(imgs_for_fake_anchor, fake_anchor_prc, axis=(0, 1),keepdims=True) - np.min(imgs_for_fake_anchor,axis=(0, 1),keepdims=True))
            # imgs_coding_tophat_norm = imgs_coding_tophat # without normalization
            anchors = np.swapaxes(np.swapaxes(imgs_for_fake_anchor_norm.max(axis=2), 0, 2), 1, 2)
        ...
        try:
            ...
            spots_i=None; centers_i=None; spots_notophat_i=None

        return spots_i, centers_i, spots_notophat_i

    input_tiles = [(x_ind, y_ind) if np.isin('X' + str(x_ind) + '_Y' + str(y_ind), tile_names['selected_tile_names']) else None for x_ind in range(tiles_to_load['x_start'], tiles_to_load['x_end'] + 1) for y_ind in range(tiles_to_load['y_start'], tiles_to_load['y_end'] + 1)]
    input_tiles = list(filter(None, input_tiles))
    num_cores = multiprocessing.cpu_count()
    processed_list = Parallel(n_jobs=num_cores)(delayed(tile_function)(i[0],i[1]) for i in input_tiles)

    for i in input_tiles:
        spots_i, centers_i, spots_notophat_i = processed_list[input_tiles.index(i)]
        N_i = spots_i.shape[0]
        if N_i > 0:
            spots = np.concatenate((spots, spots_i))
            if compute_also_without_tophat:
                spots_notophat = np.concatenate((spots_notophat, spots_notophat_i))
            # saving spots locations from the 1st cycle in a data frame (needed for ploting)
            X = (i[0] - tiles_to_load['x_start']) * tiles_info['tile_size'] + centers_i[:, 0]
            Y = (i[1] - tiles_to_load['y_start']) * tiles_info['tile_size'] + centers_i[:, 1]
            Tile = np.tile(np.array(['X' + str(i[0]) + '_Y' + str(i[1])]), N_i)
            spots_loc_i = pd.DataFrame(
                np.concatenate((X.reshape((N_i, 1)), Y.reshape((N_i, 1)), Tile.reshape((N_i, 1))), axis=1),
                columns=['X', 'Y', 'Tile'], index=None)
            spots_loc = spots_loc.append(spots_loc_i, ignore_index=True)
